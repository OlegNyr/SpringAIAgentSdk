...
public class ProcessModel {
    ...
    public ProcessedResponse processResponse(Agent agent,
        ...
        for (ResponseOutputItem item : modelResponse.output()) {
            if (item instanceof ResponseOutputMessage message) {
                runItems.add(new MessageOutputItem(agent, message));
            } else {
                if (!(item instanceof ResponseFunctionToolCall)) {
                    throw new UnsupportedOperationException();
                }
            }
            if (item instanceof ResponseFunctionToolCall toolCall) {
